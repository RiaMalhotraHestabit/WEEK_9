...
from config import (LOGS_DIR, TRACE_FILE,AGENT_ORDER, CRITIC_THRESHOLD)
import agents as ag
...
def safe_step(
    ...
    #Run one agent with full failure recovery. Appends result to trace regardless of success or failure.
    print(f"\n[NEXUS] : Running {name}...")
    try:
...
def run(query: str, verbose: bool = False) -> dict:
    ...
    for agent_name in pipeline:
        ...
        # Optimizer sees Critic feedback + everything before it
            ...
            context = output  # Every other agent just gets the last agent's output as context
